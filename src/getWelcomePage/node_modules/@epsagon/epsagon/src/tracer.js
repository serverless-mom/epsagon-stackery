 ...
 */
function sendTraceNow(traceSender) {
    const { tracer } = module.exports;
    const traceJson = {
...
/**
 * Sends the trace to epsagon's infrastructure.
...
module.exports.sendTrace = function sendTrace(runnerUpdateFunc) {
    const traceSender = (traceObject) => {
        const options = {
            json: traceObject,
            url: config.config.traceCollectorURL,
            headers: {},
            timeout: sendTimeoutMiliseconds,
        };

        utils.debugLog(util.inspect(options, { showHidden: false, depth: null }));

        return new Promise((resolve) => {
            request.post(options, (err, res, body) => {
                utils.debugLog('trace sent async!', err, body);
                if (err) {
                    resolve(err); // never reject
                } else {
                    resolve(res);
                }
            });
    ...
    return Promise.all(pendingEvents.values()).then(() => {
        ...
        runnerUpdateFunc();
        return sendTraceNow(traceSender);
    });
};
...
module.exports.sendTraceSync = function sendTraceSync() {
    const traceSender = (traceObject) => {
        const options = {
            ...
            url: config.config.traceCollectorURL,
            timeout: sendTimeoutMiliseconds,
            socketTimeout: sendTimeoutMiliseconds,
        };
        ...
        try {
            let done = false;
            request.post(options, (err, res, body) => {
                utils.debugLog('trace sent sync!', err, body);
                done = true;
            });
